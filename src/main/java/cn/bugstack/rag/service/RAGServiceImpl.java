...
import cn.bugstack.rag.adapter.RedisAdapter;
...
public class RAGServiceImpl implements RAGService {
    ...
    @Value("${spring.ai.rag.stream.stream-key:rag:ingest:stream}")
    private String streamKey;

    @Override
    public Response<QueryTagListResponse> queryRagTagList() {
        try {
            List<String> tags = redisAdapter.getAllRagTags();
    ...
    @Override
    public Response<String> uploadFiles(String ragTag, List<String> filePaths) {
        try {
            log.info("上传文件到知识库, ragTag: {}, 文件数量: {}", ragTag, filePaths.size());

            // 添加标签
            redisAdapter.addRagTag(ragTag);

            // 发布摄入任务到Redis Stream
            RStream<String, String> stream = redissonClient.getStream(streamKey);
            for (String filePath : filePaths) {
                String taskId = UUID.randomUUID().toString().replace("-", "");
                IngestTask task = IngestTask.createPending(taskId, ragTag, filePath);
            ...
            return Response.ok("文件上传成功，等待处理");
        } catch (Exception e) {
            log.error("上传文件失败", e);
    ...
    public Response<String> addKnowledge(String ragTag, String content) {
        try {
            ...
            vectorStoreAdapter.addDocuments(List.of(document));
    ...
    @Override
    public Response<RerankResponse> rerank(RerankRequest request) {
        try {
            ...
            // 2. 向量检索
            List<String> retrievedDocs = scenarios.isEmpty()
                    ? List.of()
                    : vectorStoreAdapter.similaritySearchWithDeduplication(
                            scenarios, request.getRagTag(), 10);

            log.info("向量检索完成, 文档数: {}", retrievedDocs.size());

            // 3. 使用Jina Rerank Model进行重排序
            List<String> rerankedCases = retrievedDocs.isEmpty()
                    ? List.of()
                    : rerankService.rerank(request.getContent(), retrievedDocs, 10);
    ...
    @Override
    public Response<GenerateCasesResponse> generateCases(GenerateCasesRequest request) {
        try {
            log.info("生成测试用例, ragTag: {}", request.getRagTag());
            ...
            RerankRequest rerankRequest = RerankRequest.builder()
                    ...
                    .build();
            Response<RerankResponse> rerankResp = rerank(rerankRequest);

            List<String> referenceCases = rerankResp.getData() != null
                    ? rerankResp.getData().getRerankedCases()
                    : List.of();

            // 2. 生成测试用例
            String generatePrompt = testCaseGenerateService.buildGeneratePrompt(
                    request.getContent(), referenceCases);
            String generatedContent = ThinkStreamFilter.stripThink(chat(generatePrompt));

            // 3. 解析JSON获取用例数量，并保存到向量库
            int caseCount = 0;
            try {
                var jsonNode = com.alibaba.fastjson2.JSON.parseObject(generatedContent);
                ...
                // 保存测试用例JSON到向量库
                if (caseCount > 0) {
                    redisAdapter.addRagTag(request.getRagTag());
                    vectorStoreAdapter.saveTestCases(request.getRagTag(), List.of(generatedContent));
                    log.info("测试用例已存入向量库, ragTag: {}, 数量: {}", request.getRagTag(), caseCount);
                }
            } catch (Exception e) {
                log.warn("解析/保存用例失败: {}", e.getMessage());
                caseCount = countJsonCases(generatedContent);
            }

            // 4. 构建响应
            GenerateCasesResponse response = GenerateCasesResponse.builder()
                    .casesJson(generatedContent)
                    .count(caseCount)
                    .ragTag(request.getRagTag())
                    .build();

            log.info("测试用例生成完成, 用例数量: {}", caseCount);
    ...
    public Flux<String> generateCasesStream(GenerateCasesRequest request) {
        return Flux.defer(() -> {
            try {
                ...
                // 2. 分别检索知识库文档和历史测试用例
                List<String> knowledgeDocs = vectorStoreAdapter.similaritySearchWithDeduplication(
                        scenarios, request.getRagTag(), 5);
                knowledgeDocs = filterOutTestCases(knowledgeDocs);
                List<String> testCases = vectorStoreAdapter.queryTestCases(request.getRagTag(), 10);
                List<String> rerankedCases = testCases.isEmpty()
                        ? List.of()
                        : rerankService.rerank(request.getContent(), testCases, 10);
                ...
                Flux<String> generateFlux = streamChat(generatePrompt)
                        ...
                        .flatMap(chunk -> {
                            // 缓冲并检测完整用例
                            return Flux.fromIterable(extractCompleteCases(chunk));
                        })
    ...
    private List<String> parseRerankResult(String result) {
    ...
    public Response<String> saveTestCases(String ragTag, List<String> testCasesJson) {
        try {
            ...
            // 保存到向量库
            vectorStoreAdapter.saveTestCases(ragTag, testCasesJson);
    ...
    public Response<QueryTestCaseResponse> queryTestCases(String ragTag, Integer topK) {
        try {
            ...
            List<String> testCases = vectorStoreAdapter.queryTestCases(ragTag, limit);
    ...
    public Response<String> adoptTestCase(String ragTag, String caseId) {
        try {
            ...
            vectorStoreAdapter.updateTestCaseStatus(ragTag, caseId, "ADOPTED", null);
    ...
    public Response<String> rejectTestCase(String ragTag, String caseId, String reason) {
        try {
            ...
            vectorStoreAdapter.updateTestCaseStatus(ragTag, caseId, "REJECTED", reason);
    ...
    public Response<QueryKnowledgeResponse> queryKnowledge(String ragTag, Integer topK) {
        try {
            ...
            List<QueryKnowledgeResponse.KnowledgeDoc> docs = vectorStoreAdapter.queryKnowledgeDocs(ragTag, limit);
    ...
    public Response<String> deleteKnowledge(String ragTag, String docId) {
        try {
            ...
            vectorStoreAdapter.deleteKnowledgeDoc(ragTag, docId);
    ...
    public Response<QueryTestCaseStatsResponse> queryTestCaseStats(String ragTag) {
        try {
            ...
            VectorStoreAdapter.TestCaseStats stats = vectorStoreAdapter.queryTestCaseStats(ragTag);
